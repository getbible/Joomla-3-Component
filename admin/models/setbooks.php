...
class GetbibleModelSetbooks extends JModelList
{
		...
		*/
		...
		if($items){
			foreach ($items as &$item) {
				$item->url = JRoute::_('index.php?option=com_getbible&amp;task=setbook.edit&amp;id=' .(int) $item->id);
	...
	public function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.book_name LIKE ' . $search . ' OR a.version LIKE ' . $search . ')');
			}
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.book_name');
	...
	protected function checkInNow()
	{
		...
		if ($time){
			...
			// Check table
			$query->update($db->quoteName('#__getbible_setbooks'))->set($fields)->where($conditions); 
